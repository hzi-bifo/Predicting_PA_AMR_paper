...
library(readr)
library(argparse)
miscl_df <- read_tsv("~/pseudo_genomics/results/classification/v8/misclassified/v2/miscl_all.txt")
...
p <- ggplot(data = miscl_df, aes(x = as.factor(MIC), y = value, fill = variable)) +
    geom_bar(stat = "identity") + facet_wrap(~ drug + mode) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1))+
    xlab("MIC") +
    ylab("#isolates")+
    scale_fill_brewer(labels=c("intermediate (not classified)", "misclassified", "correctly classified"), palette = "Dark2") + 
